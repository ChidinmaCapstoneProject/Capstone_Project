...
export default function Conversation({ conversation, userId }) {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      try {
        const res = await axios("/register/oneUser?userId=" + friendId);
        setUser(res.data);
      } catch (err) {
        console.log(err);
      }
